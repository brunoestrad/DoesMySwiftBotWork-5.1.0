...
public class DoesMySwiftBotWork {
    ...
    public static void testCamera(){
        try{
            /* Taking 720x720 images for the test, however, it is recommended to use smaller sizes for faster
            code execution */
            ...
            if(image == null || bwImage == null){
            	...
            	ImageIO.write(bwImage, "png", new File("/home/pi/bwImage.png"));
    ...
    /*
     * Testing each light for the SwiftBot Buttons. Each button is first individually
     *  turned on using the `testIndividualButtonLights()` method below this method. then, all button
    ...
    /*
     * This method tests each individual button light, by firstly turning them on with {@code setButtonLight();},
     * before toggling them off with {@code toggleButtonLight();}. Afterwards, a for loop is ran that will turn up
     * the brightness of the individual button light from 0 to 100 with {@code setButtonLightBrightness}, before
     * turning it off completely with {@code setButtonLight();}.
     */
    public static void testIndividualButtonLights(){
        try{
            Button buttons[] = {Button.A, Button.B, Button.X, Button.Y}; // Creating an array with all the Bot's buttons.
            ...
            for(Button button : buttons){
                ...
                // This snippet will adjust the brightness of an individual button light, from 0 (off) to 100 (on)
                for(int i = 0; i < 100; i++){
    ...
    /*
     * This method test all the button lights at once by using the {@code fillButtonLights();} method. After 2.5
     * seconds, all the button lights will be turned off with {@code disableButtonLights();}.
     */
    ...
    /*
     * Tests the user-selected wheel, taking the wheel as a parameter: either "left" or "right".
     * If the "left" wheel is selected, the function sets the left wheel velocity to 100%
     * If the "right" wheel is selected, the function sets the right wheel velocity to 100%
     */
    public static void testWheel(String wheel){
        ...
        try{
            // Moves the wheels with the set left wheel and right wheel velocity, for 3 seconds.
    ...
    /*
     * Tests the user-selected wheel defined in the method above, but reverses the wheels instead.
    ...
    /*
        Tests each button by making them all active for 10 seconds. You can extend, or shorten the time by changing
        the {@code endTime} variable. When you enable a button, you are free to encase a method inside of it that will
        only run once a button has been pressed. In this example, once a button has been pressed, it'll be disabled using
        {@code swiftBot.disableButton();}, meaning it cannot be pressed again. You are also able to disable all buttons
    ...
    /*
    This method uses a range of colours to test all under lights, where each colour is represented
    as an integer array, with the first value being the amount of red, the second the amount of green
    and the third the amount of blue. These colours are then looped through with a for loop and assigned
    ...
    /*
    This method uses three colours to test each individual under light: red, green and blue.
    An array of under lights are created, and then iterated through with a for loop.
    Inside this for loop, each under light is set to red, then 200 milliseconds later set to blue,
    then 200 milliseconds to green. After this, the colour of the under light is not removed, such that
    ...
    public static void testIndividualUnderlights() throws InterruptedException {
        ...
        try{
            ...
            Underlight[] underlights = new Underlight[] {Underlight.BACK_LEFT,Underlight.BACK_RIGHT, Underlight.MIDDLE_LEFT,
    ...
    /*
    This method takes a capture from the SwiftBot's camera to try and find a QR code in an image, and then
    decode it. If a QR code is found, the method will print the decoded message to the console.
    You could use these methods to have the SwiftBot take commands such as movement, lights, etc by scanning them!

    Note - the distance between the SwiftBot's camera and the QR code matters. If the QR code is too close or too far,
